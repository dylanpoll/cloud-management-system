...
generate ssh-key with putty in windows or using ssh-agent within linux. 
After you have a key, perform the following.
...
change contents to have/include
    ...
    UsePAM no
ctrl+x, then press y to save and apply changes 
...
Copy the following into the nginx.conf
...
after that, in order to setup the service on startup do the following.
...
Press ctrl a, let go, press d (this detaches this so you can do other things and leave it running.)
